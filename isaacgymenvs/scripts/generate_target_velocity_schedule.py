...
def generate_sigmoid_velocity_steps100(n_timesteps):
    # Change step for smaller or larger transisitons
    # vel1 = np.array([random.random(), ] * n_timesteps)
...
if __name__ == "__main__":
    ...
    _blend_schedule =False
    sigmoid_data = np.empty((num_episode,n_timesteps))
    sigmoid_data1 = np.empty((num_episode, n_timesteps))
    sigmoid_data100 = np.empty((num_episode, n_timesteps))

    for i in range(num_episode):
        sigmoid_data[i]= generate_sigmoid_velocity_steps(n_timesteps)
        sigmoid_data1[i] = generate_sigmoid_speed_schedule(n_timesteps)
        sigmoid_data100[i] = generate_sigmoid_velocity_steps100(n_timesteps)


    if _blend_schedule:
        sigmoid_data[1::3, :] = sigmoid_data1[1::3, :]
        sigmoid_data[0::3, :] = sigmoid_data100[0::3, :]
    ...
    ts = np.linspace(0,8* sigmoid_data.shape[0],sigmoid_data.shape[0]*sigmoid_data.shape[1])
    ...
    for i in range(num_episode):
        plt.axvline(len_episode*(i+1), color='black', linestyle='--')
        val = len_episode*(i+1)
        vals.append(val)
    ...
    plt.ylabel('Forward Velocity (m/s)' ,fontsize=20)
    plt.xlabel('Episode Time', fontsize=20)
    plt.xticks(vals,labels,fontsize=15)
